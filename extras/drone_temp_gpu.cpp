...
//								  scale1, scale1);
		cv::Rect section1(prev_quick[i].Pt.x - 5, prev_quick[i].Pt.y - 5, 10, 10); 
		...
		for(double scale = 1; scale < 2; scale += 0.1)
		{
			...
			double scale2 = h_currKeypoints[good_matches[i].trainIdx].size * 2.5 * scale;
	...
	for (unsigned int i = 0; i < curr_expanding.size(); i++)
		circle(sample, curr_expanding[i].pt, 6, Scalar(2, 25, 200), -1);
	//drawKeypoints(sample, expanding, sample, Scalar(2, 25, 200), );
