...
/* Function for template matching - Detection and Avoidance*/
void getTemplate(Mat, Mat, vector<KeyPoint>, vector<KeyPoint>, vector<DMatch>&, double&, double&, double&, double&);
...
int main(int argc, char* argv[])
{
	...
	try
	{
		...
		for (;;)
		{
			...
			drawKeypoints(currFrame, h_currKeypoints, currFrame, Scalar(255, 255, 255));
...
*/
					...
					double vx = 1.0, vy = 0.0, vz = 0.0, vr = 0.0;
					getTemplate(prevFrame, currFrame, h_prevKeypoints, h_currKeypoints, good_matches,vx, vy,vz, vr);
...
*/
			...
			for (int i = 0; i < h_currKeypoints.size(); i++)
				h_prevKeypoints.push_back(h_currKeypoints[i]);
...
//	Mat keyframe;
//	drawKeypoints( frame, frame, keys, Scalar::all(-1), DrawMatchesFlags::DEFAULT );
//	imshow("keys", keyframe);
...
void getTemplate(Mat prevFrame, Mat currFrame, vector<KeyPoint> h_prevKeypoints, vector<KeyPoint> h_currKeypoints, vector<DMatch>& good_matches, double & vx, double & vy, double & vz, double & vr)
{
	...
	/* Template Matching */
	for(unsigned int i = 0; good_matches.size(); i++)
	{
//		double scale1 = h_prevKeypoints[good_matches[i].queryIdx].size * 2.5;
//		Mat PrevTemp(prevFrame, cv::Rect(prev_quick[i].Pt.x - scale1/2, prev_quick[i].Pt.y - 
//							scale1/2, scale1, scale1));
...
/*
		for(int scale = 1; scale < 1.5; scale += 0.1)
		{
