...
void getFeatures(Ptr<SURF>, Mat, Mat&, vector<KeyPoint>&);
...
//		GpuMat d_threshold;
		...
		Ptr<cv::ximgproc::segmentation::GraphSegmentation> seg = cv::ximgproc::segmentation::createGraphSegmentation(0.8, 300, 100);
...
*/
void getFeatures(Ptr<SURF> detector, Mat frame, Mat& descriptors, vector<KeyPoint>& keys)
{
        ...
        detector->compute(frame, keys, descriptors);
	//cout << "Detecting...: " << keys.size() << endl;
...
void filter(vector<DMatch> matches, vector<DMatch>& good_matches, vector<KeyPoint> prevKeys, vector<KeyPoint> currKeys)
{
	...
	for (unsigned int i = 0; i < matches.size(); i++)
	{
		...
		if (matches[i].distance < 2*min_dist  && ratio > 1.05 )
